...
void ofApp::draw() {
	...
	for (int row = 0; row < this->matrix.size(); row++) {
		...
		for (int col = 0; col < this->matrix[row].size(); col++) {
			...
			int y = col * this->font_size;

			if (this->params[row][col] < 255) {
			
				this->matrix[row][col] = this->words[(int)(ofRandom(this->words.size()))];
			}

			ofSetColor(23, this->params[row][col]);
			...
			this->params[row][col] = (this->params[row][col] + 1) % 576;
		}
	}
}
